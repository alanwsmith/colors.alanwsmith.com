...
function getClassMemberInfos(classDeclaration, sourceCode, orderedSlots) {
  ...
  var members = classMemberNodes.filter(function (x) {
    return x.key;
...
function getMemberInfo(node, sourceCode) {
  ...
  var _abstract = node.type === 'TSAbstractAccessorProperty' || node.type === 'TSAbstractPropertyDefinition' || node.type === 'TSAbstractMethodDefinition';
  ...
  if (node.type === 'ClassProperty' || node.type === 'ClassPrivateProperty' || node.type === 'PropertyDefinition' || node.type === 'PrivateIdentifier' || node.type === 'TSAbstractPropertyDefinition' || node.type === 'TSPropertySignature') {
    ...
    if (isPrivate) {
      name = "#".concat(node.key.id ? node.key.id.name : node.key.name);
    } else {
    ...
    if (node.computed) {
      ...
      name = isPrivate ? "#".concat(node.key.id ? node.key.id.name : node.key.name) : node.key.name;
    }
...
function expandSlot(input, groups) {
  ...
  var testName = slot.name && getStringComparer(slot.name);
...
function getStringComparer(str) {
  if (str[0] === '/') {
    var strPattern = str.substr(1, str.length - 2);
...
var comparers = [{
  ...
  value: 10,
  test: function test(m, s) {
    return s["abstract"] == m["abstract"];
  }
}, {
  property: 'override',
  value: 10,
  test: function test(m, s) {
    return s.override == m.override;
  }
}, {
  property: 'readonly',
  value: 10,
  test: function test(m, s) {
    return s.readonly == m.readonly;
  }
}, {
  property: 'kind',
  value: 10,
  ...
  test: function test(m, s) {
    if (typeof s.groupByDecorator === 'boolean') {
      var hasDecorators = m.decorators.length > 0;
      return s.groupByDecorator === hasDecorators;
    } else {
      var comparer = getStringComparer(s.groupByDecorator);
      return m.decorators.some(function (decorator) {
        return comparer(decorator);
      });
    }
  }
}, {
